...
function addActionToSlice(existingCode, modelName, actionName) {
...
function addActionToSaga(existingCode, modelName, actionName) {
    const ast = recast.parse(existingCode, {
        parser: {
            parse(source) {
                return babelParser.parse(source, {
...
function addThunkActionToSlice(existingCode, newThunkCode, newExtraReducersCode) {
    const ast = recast.parse(existingCode, {
        parser: {
            parse(source) {
                return babelParser.parse(source, {
    ...
    recast.visit(ast, {
        visitCallExpression(path) {
            if (recast.types.namedTypes.Identifier.check(path.node.callee) &&
                ...
                const extraReducersProperty = sliceConfig.properties.find((prop) => recast.types.namedTypes.ObjectProperty.check(prop) &&
                ...
                if (extraReducersProperty &&
                    ...
                    recast.types.namedTypes.ArrowFunctionExpression.check(extraReducersProperty.value)) {
