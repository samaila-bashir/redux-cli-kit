 ...
 */
async function createDirectories(modelName: string) {
 ...
 */
async function createSliceFile(
...
async function createStoreStructure(
  ...
  const { srcDir, slicesDir, modelDir, componentsDir } =
    await createDirectories(modelName);
  const modelNameCapitalized =
    modelName.charAt(0).toUpperCase() + modelName.slice(1);
