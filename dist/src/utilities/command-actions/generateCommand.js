...
async function ensureConfig() {
    ...
    if (!config) {
        ...
        config = { framework, stateManagement, storeDir: specifiedDir };
 ...
 */
function setupDirectories(config, modelName) {
...
export async function generateCommand(modelName, options) {
    ...
    const { sliceDir, sagaDir, sliceFilePath, sagaFilePath } = setupDirectories(config, modelName);
    ...
    if (!options.slice && !options.saga && !options.thunk) {
        await generateFullCRUD(config, modelName, sliceFileExists, sliceDir, sagaDir, sliceFilePath, sagaFilePath, customSlicePath);
    }
    else {
        if (options.slice) {
            await generateSlice(config, options, modelName, sliceFileExists, sliceDir, sliceFilePath);
        }
        if (options.saga &&
            config.stateManagement === 'reduxSaga' &&
            sagaDir &&
            sagaFilePath) {
            await generateSaga(config, options, modelName, sagaFileExists, sagaDir, sagaFilePath, customSlicePath);
        }
        if (options.thunk || config.stateManagement === 'reduxThunk') {
            await generateThunk(config, options, modelName, sliceFileExists, sliceDir, sliceFilePath);
        }
    }
