...
import { readConfigFile, writeConfigFile, } from '../../../../utilities/helpers/config.js';
...
import { chooseFramework, chooseStateManagement, } from '../../../../utilities/helpers/utils.js';
// Function to generate slice, saga, or thunk based on user input
export async function generateModel(modelName, options) {
    ...
    if (!options.slice && !options.saga && !options.thunk) {
        ...
        if (stateManagement === 'reduxSaga') {
            ...
            await fs.writeFile(path.join(sagaDir, 'index.ts'), generateModelSagaCRUD(modelName, customSlicePath) // Full CRUD for Saga
            );
        ...
        if (options.saga && options.action) {
            ...
            await fs.ensureDir(sagaDir);
            await fs.writeFile(path.join(sagaDir, 'index.ts'), generateSingleActionSaga(modelName) // Single Action Saga
            );
    ...
    if (config) {
        ...
        console.log(chalk.blue('Reminder: Update your actions and watchers as necessary.'));
    }
}
