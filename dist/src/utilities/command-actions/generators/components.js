 ...
 */
export async function generateSelectedComponents(context, options) {
    if (options.slice) {
        await generateSlice(context.config, options, context.modelName, context.sliceFileExists, context.sliceDir, context.sliceFilePath);
    }
    if (options.saga &&
        context.config.stateManagement === 'reduxSaga' &&
        context.sagaDir &&
        context.sagaFilePath) {
        await generateSaga(context.config, options, context.modelName, context.sagaFileExists, context.sagaDir, context.sagaFilePath, context.customSlicePath);
    }
    if (options.thunk || context.config.stateManagement === 'reduxThunk') {
        await generateThunk(context.config, options, context.modelName, context.sliceFileExists, context.sliceDir, context.sliceFilePath);
    }
}
/**
 ...
 */
export function isFullCRUDGeneration(options) {
