...
export async function generateSliceAndSaga(modelName, middleware) {
    ...
    const sliceDir = path.join(process.cwd(), `src/store/slices/${modelNameLowerCase}`);
    const sagaDir = path.join(process.cwd(), `src/store/sagas/${modelNameLowerCase}`);
    // Create the slice directory
    await fs.ensureDir(sliceDir);
    // Write the slice file using the template (single function)
    await fs.writeFile(path.join(sliceDir, 'index.ts'), generateModelSlice(modelNameCapitalized));
    // Create the saga directory and file only if Saga is chosen
    if (middleware === 'reduxSaga') {
        await fs.ensureDir(sagaDir);
        await fs.writeFile(path.join(sagaDir, 'index.ts'), generateSingleActionSaga(modelNameCapitalized));
