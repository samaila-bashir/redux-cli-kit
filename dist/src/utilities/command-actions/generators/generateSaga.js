 ...
 */
export async function generateSaga(config, options, modelName, sagaFileExists, // Add this parameter
sagaDir, sagaFilePath, customSlicePath) {
    ...
    if (sagaFileExists) {
        if (options.action) {
            ...
            const updatedSagaCode = addActionToSaga(existingSagaCode, modelName, options.action);
            await fs.writeFile(sagaFilePath, updatedSagaCode);
            console.log(chalk.green(`Action '${options.action}' added to existing saga for ${modelName}.`));
        }
        else {
            console.log(chalk.yellow(`Saga for model ${modelName} already exists. Use --action to add a new action.`));
        }
    }
    else {
        if (options.action) {
            ...
            console.log(chalk.green(`Saga for ${modelName} with action '${options.action}' created.`));
        }
        else {
